...
export function HotelListPage() {
  ...
  const fetchHotels = useCallback(
    async (targetPage: number, reset: boolean) => {
      const keywordSearch = Boolean(keyword.trim());
      const requestPage = keywordSearch ? 1 : targetPage;
      const requestLimit = keywordSearch ? 200 : PAGE_SIZE;
      ...
      try {
        ...
        const filtered = keywordSearch
          ? filteredAll.slice((targetPage - 1) * PAGE_SIZE, targetPage * PAGE_SIZE)
          : filteredAll;
        const total = keywordSearch ? filteredAll.length : response.pagination.total;
